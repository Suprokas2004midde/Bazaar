...
import {
  ...
  allProductService,
  bestSellerService,
  getBulkProductsService,
  latestCollectionService,
  listproductService,
...
export const reviewProduct = async (req, res) => {
  try {
    const { userId, productId, star, reviewText, action } = req.body;
    const response = await reviewProductService(userId, productId, star, reviewText, action);
    return res.status(200).json({
      success: true,
...
export const singleProduct = async (req, res) => {
  try {
    const response = await singeProductService(req.body._id, req.body.userId);
    return res.status(200).json({
      success: true,
