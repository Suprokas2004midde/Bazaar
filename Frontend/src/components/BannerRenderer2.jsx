...
const BannerRenderer = ({ banner }) => {
  ...
  const Eyebrow = ({ children }) =>
    children ? (
      <span className={`inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] ${tone.faint}`}>
  ...
  const renderFloatingProduct = () => (
    <div className="relative flex w-full h-full flex-col items-center justify-center gap-3 p-6 text-center sm:p-10">
      <div className="relative flex items-center justify-center h-[35%] sm:h-[40%] md:h-[45%]">
        <div className="absolute h-28 w-28 sm:h-40 sm:w-40 md:h-56 md:w-56">
          <RingPulse />
        </div>
        <Glow className="h-40 w-40 sm:h-52 sm:w-52 md:h-72 md:w-72" />
        <Picture className="relative h-full object-contain" />
      </div>
      <Eyebrow>{badge}</Eyebrow>
      <h1 className={`max-w-xl text-xl sm:text-2xl md:text-3xl font-semibold leading-tight tracking-tight ${tone.heading} line-clamp-1`}>
        {title}
      </h1>
      {subtitle && <p className={`max-w-md text-xs sm:text-sm md:text-base line-clamp-1 ${tone.body}`}>{subtitle}</p>}
  ...
  const renderGroceryFresh = () => (
    <div className="flex w-full h-full flex-col md:flex-row items-center justify-between gap-6 p-6 sm:p-10">
      <div className={`z-10 flex flex-col items-center gap-3 rounded-2xl border p-4 sm:p-6 text-center backdrop-blur-sm md:w-[48%] md:items-start md:text-left w-full h-[55%] md:h-auto md:max-h-full overflow-hidden ${tone.hairline} ${tone.surface}`}>
        <Eyebrow>{badge || "Featured"}</Eyebrow>
        <h1 className={`text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold leading-tight tracking-tight ${tone.heading} line-clamp-1 md:line-clamp-2`}>
          {title}
        </h1>
        {subtitle && <p className={`max-w-sm text-xs sm:text-sm line-clamp-1 ${tone.body}`}>{subtitle}</p>}
        <div className="pt-0.5">
          <CtaSolid>
      ...
      <div className="relative flex items-center justify-center md:w-[52%] w-full h-[40%] md:h-full max-h-[120px] sm:max-h-[160px] md:max-h-full">
        <CircleFrame />
